...
public class MallSearchServiceImpl implements MallSearchService {
    ...
    public SearchResult search(SearchParam param) {
        ...
        try {
            //执行检索请求
            SearchResponse response = restHighLevelClient.search(searchRequest, GulimailElasticSearchConfig.COMMON_OPTIONS);
    ...
    private SearchRequest bulidSearchRequest(SearchParam param) {
        ...
        //1. 构建bool-query bool （布尔）过滤器。 这是个 复合过滤器（compound filter） ，它可以接受多个其他过滤器作为参数，并将这些过滤器结合成各式各样的布尔（逻辑）组合
        BoolQueryBuilder boolQueryBuilder = new BoolQueryBuilder();
        //1.1 bool-must 用来做模糊匹配
        ...
        if (param.getAttrs() != null && param.getAttrs().size() > 0) {
            param.getAttrs().forEach(item -> {
                BoolQueryBuilder boolQuery = QueryBuilders.boolQuery();//构建bool查询
                ...
                NestedQueryBuilder nestedQueryBuilder = QueryBuilders.nestedQuery("attrs", boolQuery, ScoreMode.None);//ScoreMode.None不按分进行排序
        ...
        //2. 按照属性信息进行聚合
        NestedAggregationBuilder attr_agg = AggregationBuilders.nested("attr_agg", "attrs");//大聚合
        //2.1 按照属性ID进行聚合
        TermsAggregationBuilder attr_id_agg = AggregationBuilders.terms("attr_id_agg").field("attrs.attrId");//小聚和
...
//构建返回请求结果
    ...
    private String replaceQueryString(SearchParam param, String attr, String attrs) {
